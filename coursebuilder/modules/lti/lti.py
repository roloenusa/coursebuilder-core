...
also set two specific LTI fields:
    ...
    1. Resource Link ID. This uniquely identifies the embed to the tool
       provider. By default we set this to your course's slug; you can override
       this value if your tool provider requires something different.
...
class _CourseBuilderDispatcher(_Dispatcher):
    ...
    @classmethod
    def dispatch(cls, handler, runtime, resource_id):
        # In CB, the resource id is the path component, minus the course slug,
...
class _Parser(object):
    ...
    def parse(cls, raw_value, errors):
        """Returns a map of identifier -> DTO; populates errors string list."""
        raise NotImplementedError
    ...
    def parse(cls, raw_value, errors):
        ...
        for entry in loaded:
            ...
            try:
                ...
                # Deliberately catching everything else as a parse error.
                errors.append(cls.PARSE_ERROR)
                return
...
class _SecurityParser(_Parser):
    ...
    @classmethod
    def parse(cls, raw_value, errors):
        """Security yaml validator; returns {key_unicode: _SecurityConfig}."""
        if cls._is_valid_empty_value(raw_value):
            return {}
        ...
        for entry in loaded:
            ...
            # Cast key to unicode because later comparisons are done against
            # POSTed data (strings). Do not cast the value in the config.
...
class _Runtime(object):
    ...
    def get_login_url(self, return_url):
        ...
        return users.create_login_url(dest_url='%s?%s' % (
            _urljoin(self.get_base_url(), _REDIRECT_URL),
    ...
    def _get_configs(self, config_key, parser):
        # This may crash if users have edited config.yaml by hand. Bad
        # config.yaml edits can cause all manner of insanity; we allow errors to
        # percolate up rather than trying to recover so admins discover them
        # fast. The only case we'll handle specially is if the value is sane but
        # fails validation.
        errors = []
        ...
        if errors:
            raise ValueError(
                'Errors in %s.parse: %s' % (parser.__name__, errors))
...
class LaunchHandler(_BaseHandler):
    ...
    def get_custom_unsigned_launch_parameters(self, from_dict, name):
        ...
        Args:
            from_dict: dict of string -> value. The LTI launch request payload
                that we are calculating, modified in place.
            name: string. The name of the LTI tool configuration we're
                processing a request for.
        """
        pass

    def _get_unsigned_launch_parameters(
            self, extra_fields, name, return_url, resource_link_id, url,
            user_id):
        from_dict = {
            fields.LAUNCH_PRESENTATION_RETURN_URL: return_url,
            fields.RESOURCE_LINK_ID: resource_link_id,
...
class ValidationHandler(_BaseHandler):
    ...
    def post(self):
        ...
        if self._needs_login(
            ...
            self.redirect(
                self._get_login_redirect_url(
                    runtime.get_base_url(), return_url))
            return
...
def _get_provider_enabled_field(unused_course):
    ...
    return schema_fields.SchemaField(
        ...
        description='Whether or not to allow LTI consumers to embed content '
        'for this course. Note that the admin for this Course Builder '
        'deployment must have enabled %s for this setting to take effect, and '
        'you will also need to create a key and secret for each consumer who '
        'you want to allow to embed content from this course (see "LTI '
        'security").' % (COURSES_CAN_ENABLE_LTI_PROVIDER.name))
...
'''))
    ...
    return schema_fields.SchemaField(
        ...
        'pairs that allow other systems to make LTI launch requests against '
        'this course in your Course Builder deployment. They are very '
        'sensitive and should be treated with care (for example, emailing them '
        'will compromise the integrity and security of your user data). Each '
        'key must be unique within a course; it is used to uniquely identify '
        'the LTI consumer making a launch request. The secret, which must also '
...
def _get_tool_field(unused_course):
    tool_example = safe_dom.NodeList().append(
        safe_dom.Element('pre').add_text('''
- name: my_tool
  description: My Tool
  url: http://launch.url
...
def register_module():
    ...
    custom_module = custom_modules.Module(
        MODULE_TITLE, 'LTI module', global_handlers, namespaced_handlers,
